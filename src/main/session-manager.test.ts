...
// --- RSMR: resume on respawn — capture, persist, inject ---

/** opencode is not in the main-branch seed; register it for id-mechanism tests. */
function withOpencode(config: ConfigStore): void {
  config.patch({ agents: [...SEEDED_AGENTS, { name: 'opencode', command: 'opencode', args: [] }] })
}
...
describe('SessionManager — resume capture (RSMR-01..04, RSMR-18)', () => {
  it('retains the id printed by an id-mechanism agent and persists it on stop', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    const view = manager.spawn('opencode', CWD)
    port.handles[0].emitData('done\nopencode --session ses_f7273a311ffeAkg12W9sbfYKpt\r')
    manager.stop(view.id)
    expect(config.get().sessions[0].agentSessionId).toBe('ses_f7273a311ffeAkg12W9sbfYKpt')
  })

  it('catches a resume hint split across output chunks', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    const view = manager.spawn('opencode', CWD)
    port.handles[0].emitData('opencode --session ses_')
    port.handles[0].emitData('abc123')
    manager.stop(view.id)
    expect(config.get().sessions[0].agentSessionId).toBe('ses_abc123')
  })

  it('ignores ANSI escapes painted around the hint', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    const view = manager.spawn('opencode', CWD)
    port.handles[0].emitData('\x1b[31mopencode --session ses_ansi\x1b[0m\r')
    manager.stop(view.id)
  ...
  it('persists the id on a natural agent exit (onExit), not only explicit stop', () => {
    ...
    port.handles[0].emitData('opencode --session ses_exit\r')
    port.handles[0].emitExit(0)
    expect(config.get().sessions[0].agentSessionId).toBe('ses_exit')
  })

  it('killAll persists a running session retained id before the PTY is killed', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    manager.spawn('opencode', CWD)
    port.handles[0].emitData('opencode --session ses_quit\r')
    manager.killAll()
    expect(port.handles[0].killed).toBe(true)
  ...
  it('keeps the field absent when no id ever appears (RSMR-08)', () => {
    ...
    withOpencode(config)
    manager.spawn('opencode', CWD)
    port.handles[0].emitData('just some output')
    manager.stop(manager.list()[0].id)
    expect(config.get().sessions[0].agentSessionId).toBeUndefined()
  })
})

describe('SessionManager — resume injection on respawn (RSMR-05..07)', () => {
  it('respawn of an id-agent injects its own captured id', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    const view = manager.spawn('opencode', CWD)
    port.handles[0].emitData('opencode --session ses_own\r')
    manager.stop(view.id)
    expect(config.get().sessions[0].agentSessionId).toBe('ses_own')
    manager.respawn(view.id)
    expect(port.handles[1].plan.autoCommand).toBe('opencode --session ses_own')
  })

  it('respawn of a continue agent injects --continue', () => {
    const { manager, port } = makeManager()
    const view = manager.spawn('Claude', CWD)
    manager.stop(view.id)
    manager.respawn(view.id)
    expect(port.handles[1].plan.autoCommand).toBe('claude --continue')
  })

  it('respawn of an agent with no mechanism stays byte-identical to today', () => {
    const { manager, port } = makeManager()
    const view = manager.spawn('Codex', CWD)
    manager.stop(view.id)
    manager.respawn(view.id)
...
describe('SessionManager — resume injection on new spawn (RSMR-09..12, RSMR-23)', () => {
  it('a new id-agent spawn in a cwd resumes the last captured id of the same agent', () => {
    const { manager, config, port } = makeManager({
      seed: [stopped('s1', 'opencode', CWD, { agentSessionId: 'ses_prior' })]
    })
    withOpencode(config)
    manager.spawn('opencode', CWD)
    expect(port.handles[0].plan.autoCommand).toBe('opencode --session ses_prior')
  })

  it('never injects a different agent captured id', () => {
    const { manager, config, port } = makeManager({
      seed: [stopped('s1', 'Claude', CWD, { agentSessionId: 'ses_other' })]
    })
    withOpencode(config)
    manager.spawn('opencode', CWD)
    expect(port.handles[0].plan.autoCommand).toBe('opencode')
  })

  it('ignores a prior session in a different cwd', () => {
    const { manager, config, port } = makeManager({
      seed: [stopped('s1', 'opencode', 'C:\\work\\elsewhere', { agentSessionId: 'ses_far' })]
    })
    withOpencode(config)
    manager.spawn('opencode', CWD)
    expect(port.handles[0].plan.autoCommand).toBe('opencode')
  ...
  it('a new id-agent spawn with no prior session starts fresh', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    manager.spawn('opencode', CWD)
    expect(port.handles[0].plan.autoCommand).toBe('opencode')
  ...
  it('a new continue-agent spawn continues when the cwd has a prior session', () => {
    const { manager, port } = makeManager({ seed: [stopped('s1', 'Claude', CWD)] })
    manager.spawn('Claude', CWD)
    expect(port.handles[0].plan.autoCommand).toBe('claude --continue')
  })

  it('a new continue-agent spawn with no prior session starts fresh', () => {
    const { manager, port } = makeManager()
    manager.spawn('Claude', CWD)
    expect(port.handles[0].plan.autoCommand).toBe('claude')
  })
})

describe('SessionManager — duplicate, rename, remove (RSMR-21/22/23)', () => {
  it('duplicate never inherits the captured id and starts a fresh conversation', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    const view = manager.spawn('opencode', CWD)
    port.handles[0].emitData('opencode --session ses_dup\r')
    manager.stop(view.id)
    const clone = manager.duplicate(view.id)
    const cloned = config.get().sessions.find((s) => s.id === clone.id)
    expect(cloned?.agentSessionId).toBeUndefined()
    expect(port.handles.at(-1)?.plan.autoCommand).toBe('opencode')
  })

  it('rename keeps the captured id', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    const view = manager.spawn('opencode', CWD)
    port.handles[0].emitData('opencode --session ses_ren\r')
    manager.stop(view.id)
    manager.rename(view.id, 'My session')
    expect(config.get().sessions[0].agentSessionId).toBe('ses_ren')
  })

  it('remove drops the session and its captured id with it', () => {
    const { manager, config, port } = makeManager()
    withOpencode(config)
    const view = manager.spawn('opencode', CWD)
    port.handles[0].emitData('opencode --session ses_rem\r')
    manager.stop(view.id)
    manager.remove(view.id)
    expect(config.get().sessions).toHaveLength(0)
  })
